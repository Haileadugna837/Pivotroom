...
export function FoundingExpertsManifesto() {
  return (
    <section className="border-t border-pivot-line bg-pivot-paper-2 px-6 py-20 text-center">
      <motion.h2
        initial={{ opacity: 0, y: 12 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.5 }}
        transition={{ duration: 0.5 }}
        className="mx-auto max-w-2xl font-serif text-3xl leading-tight font-normal text-pivot-ink sm:text-4xl"
      >
      ...
      >
        {PARAGRAPHS.map((paragraph) => (
          <p key={paragraph} className="text-sm text-pivot-ink-2 sm:text-base">
            {paragraph}
          </p>
        ))}
      </motion.div>
      ...
      <motion.div
        ...
        className="mx-auto mt-8 max-w-lg rounded-2xl border-l-4 border-pivot-accent bg-pivot-white p-6 text-left"
      >
        <p className="text-base font-medium text-pivot-ink">
