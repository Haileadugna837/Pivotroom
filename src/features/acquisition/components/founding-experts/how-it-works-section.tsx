  ...
  return (
    <section className="border-t border-white/10 bg-white/[0.02] px-6 py-20 text-center">
      ...
      <div className="relative mx-auto mt-14 grid max-w-4xl grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4">
        <div
          aria-hidden="true"
          className="absolute top-7 right-0 left-0 hidden h-px bg-white/10 sm:block"
        ...
        {STEPS.map((step, i) => {
          ...
          return (
            ...
            >
              <span className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full border-2 border-white/30 bg-neutral-900">
                <Icon className="h-5 w-5" strokeWidth={1.75} aria-hidden="true" />
                <span className="absolute -top-1.5 -right-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-white text-[11px] font-semibold text-black">
                  {step.n}
                </span>
              </span>
              <p className="max-w-[10rem] text-sm font-medium text-white/80">{step.text}</p>
            </motion.div>
          );
        })}
      </div>
